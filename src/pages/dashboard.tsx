...
import { getSpecialOffers } from 'apiSdk/special-offers';
...
import { CustomerFeedbackListPage as ShowCaseList } from 'pages/customer-feedbacks';

function DashboardPage() {
  const { session } = useSession();

  const specialOffersStat = useSWR<PaginatedInterface<SpecialOffersInterface>>(
    () => `/special-offers`,
    () => getSpecialOffers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: specialOffersStat?.data?.totalCount,
      isLoading: specialOffersStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`customer-feedbacks`} passHref legacyBehavior>
                  <Button
